    ...
    T: MatrixElement + std::fmt::Debug,
{
    ...
    where
        T: super::ToleranceOps
            ...
            + Neg<Output = T>
            + Clone
            + super::norm::Abs<Output = T::Abs>,
        T::Abs: MatrixElement + Add<Output = T::Abs> + Mul<Output = T::Abs> + PartialOrd + super::NanCheck,
    {
    ...
    where
        T: Div<Output = T> + Mul<Output = T> + Sub<Output = T> + Add<Output = T> + Neg<Output = T> + super::ToleranceOps,
        T::Abs: MatrixElement + Add<Output = T::Abs> + Mul<Output = T::Abs> + PartialOrd,
    {
        match N {
    ...
    where
        T: Mul<Output = T> + Sub<Output = T> + Div<Output = T> + Add<Output = T> + Neg<Output = T> + super::ToleranceOps,
        T::Abs: PartialOrd,
    {
        // Compute determinant
        ...
        T::Abs: PartialOrd,
    {
        ...
        let self_3x3: &ConstMatrix<T, 3, 3> = unsafe { &*(self as *const _ as *const ConstMatrix<T, 3, 3>) };
        let inv = self_3x3.inverse_with_tol(tolerance).ok_or(MatrixError::DimensionMismatch)?;

        // Cast back to N=3 for multiplication
        let inv_nxn: &ConstMatrix<T, N, N> = unsafe { &*(&inv as *const _ as *const ConstMatrix<T, N, N>) };
        Ok(inv_nxn * b)
    }
        ...
        T::Abs: PartialOrd,
    {
        ...
        let self_4x4: &ConstMatrix<T, 4, 4> = unsafe { &*(self as *const _ as *const ConstMatrix<T, 4, 4>) };
        let inv = self_4x4.inverse_with_tol(tolerance).ok_or(MatrixError::DimensionMismatch)?;
